...
echo "self-check:"
curl -s -H "$AUTH" -X POST localhost:$PORT/verify/run | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const r=JSON.parse(s);console.log(`offset ${r.at_offset}: ledger ${r.ledger_count} vs mirror ${r.mirror_count}, missing ${r.only_in_ledger}, phantom ${r.only_in_mirror}, ${r.duration_ms} ms`)})'
